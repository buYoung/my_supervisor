...
const ROOT_OWNER_MARKER: &str = ".release-quality-root-uuid";
...
impl TestRoot {
    fn cleanup(&mut self) {
        cleanup_owned_root(&self.path, &self.marker).expect("owned recovery root cleanup succeeds");
        assert!(
            !self.path.exists(),
            "owned recovery root is removed after explicit cleanup"
        );
    }
}
...
impl Drop for TestRoot {
    fn drop(&mut self) {
        if let Err(error) = cleanup_owned_root(&self.path, &self.marker) {
            eprintln!(
                "release-quality recovery root cleanup skipped: path={} error={error}",
                self.path.display()
            );
        }
    }
...
fn cleanup_owned_root(path: &Path, marker: &str) -> std::io::Result<()> {
    if !path.exists() {
        return Ok(());
    }
    if !std::fs::symlink_metadata(path)?.file_type().is_dir() {
        return Err(std::io::Error::other("recovery root is not a directory"));
    }
    let marker_path = path.join(ROOT_OWNER_MARKER);
    if !std::fs::symlink_metadata(&marker_path)?
        .file_type()
        .is_file()
        || std::fs::read_to_string(&marker_path)? != marker
    {
        return Err(std::io::Error::new(
            std::io::ErrorKind::PermissionDenied,
            "recovery root ownership marker does not match",
        ));
    }
...
fn test_root() -> TestRoot {
    ...
    std::fs::create_dir(&path).expect("create unique recovery root");
    std::fs::set_permissions(&path, std::fs::Permissions::from_mode(0o700))
        .expect("restrict recovery root permissions");
    let marker_path = path.join(ROOT_OWNER_MARKER);
    let mut marker_file = std::fs::OpenOptions::new()
        .create_new(true)
        .write(true)
        .open(marker_path)
        .expect("create recovery root ownership marker");
    std::io::Write::write_all(&mut marker_file, marker.as_bytes())
        .expect("write recovery root ownership marker");
    println!(
        "release-quality recovery root: path={} uuid={marker}",
        path.display()
    );
    TestRoot { path, marker }
}
...
fn daemon(root: &PathBuf, port: u16) -> Command {
    ...
    command
        ...
        .env(
            "MSV_DAEMON_TEST_CONTROL_SOCKET",
            root.join("private-control.sock"),
        )
        .kill_on_drop(true)
...
async fn wait_for_health(root: &Path, port: u16) {
    for _ in 0..100 {
        if request(root, port, "GET", "/api/v1/health", None)
            .await
...
async fn request(root: &Path, port: u16, method: &str, path: &str, body: Option<&str>) -> String {
    let body = body.unwrap_or("");
    let mut stream = match TcpStream::connect((std::net::Ipv4Addr::LOCALHOST, port)).await {
        Ok(stream) => stream,
        Err(_) => return String::new(),
    };
    let token = tokio::fs::read_to_string(root.join("run/control.token"))
        .await
        .unwrap_or_default();
    let request = format!(
...
async fn arm(root: &Path, command: &str) {
...
async fn follow_events(root: &PathBuf, port: u16) -> EventFollow {
...
async fn wait_for_terminal_event_line(follow: &EventFollow) -> serde_json::Value {
    for _ in 0..120 {
        ...
        let _ =
            tokio::time::timeout(Duration::from_millis(100), follow.line_notify.notified()).await;
    }
    panic!("CLI did not record the first durable terminal event before timeout");
}

async fn wait_for_pending_terminal_event(
    store: &SqliteStore,
) -> my_supervisor_core::ports::TransientTerminalEvent {
    for _ in 0..120 {
...
async fn sigkill_restart_replays_terminal_outbox_to_one_cli_json_line_and_converges() {
    ...
    wait_for_health(&root, port).await;
    let follow = follow_events(&root, port).await;
    ...
    assert!(request(
        ...
        port,
        "POST",
        "/api/v1/jobs/terminal-replay/trigger",
        None
    )
    .await
    ...
    assert_eq!(
        store.list_runs("terminal-replay", 10).await.unwrap().len(),
        1
    );
    let pending_event = wait_for_pending_terminal_event(&store).await;
    assert_eq!(
        pending_event.event_id.to_string(),
        first_event_id,
        "CLI first frame and durable outbox share the stable event ID"
    );
    ...
    assert!(
        ...
        "CLI event follow exits successfully: {}",
        String::from_utf8_lossy(&output.stderr)
    );
    assert!(
        output.stderr.is_empty(),
        "CLI event follow keeps stderr empty"
    );
    let matching_events = recorded_lines
        .iter()
        .map(|line| {
            serde_json::from_str::<serde_json::Value>(line).expect("CLI event line is JSON")
        })
    ...
    assert_eq!(
        matching_events.len(),
        1,
        "CLI session dedupe prints replayed terminal event once"
    );
    root.cleanup();
    assert!(process_is_gone(first_pid), "first daemon process is reaped");
    assert!(
        process_is_gone(second_pid),
        "second daemon process is reaped"
    );
}
...
async fn sigkill_restart_completes_pre_cancellation_delete_rollback_without_losing_queued_runs() {
    ...
    assert!(request(
        ...
        port,
        "POST",
        "/api/v1/jobs/delete-rollback/trigger",
        None
    )
    .await
    .starts_with("HTTP/1.1 202"));
    ...
    let deletion = request(
        ...
        port,
        "DELETE",
        "/api/v1/jobs/delete-rollback?force=true",
        None,
    )
    .await;
    assert!(
        deletion.starts_with("HTTP/1.1 500"),
        "expected injected rollback clear failure: {deletion}"
    );
    ...
    let store = SqliteStore::connect(root.join("state.db")).await.unwrap();
    assert_eq!(
        store.list_incomplete_job_deletions().await.unwrap().len(),
        1
    );
    assert_eq!(
        store.list_runs("delete-rollback", 10).await.unwrap().len(),
        2
    );
    drop(store);
    ...
    assert_eq!(
        terminal_events.len(),
        1,
        "restart cancellation commits its terminal outbox event"
    );
    assert_eq!(
...
async fn sigkill_restart_forward_recovers_direct_config_snapshot_after_commit_failure() {
    ...
    let reload = request(&root, port, "POST", "/api/v1/daemon/reload", None).await;
    assert!(
        !reload.starts_with("HTTP/1.1 200"),
        "expected injected config commit failure: {reload}"
    );
    let store = SqliteStore::connect(root.join("state.db")).await.unwrap();
    assert_eq!(
        store.list_incomplete_config_applies().await.unwrap().len(),
        1
    );
    drop(store);
