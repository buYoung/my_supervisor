...
async fn durable_sequences_continue_after_restart_and_seal_rejects_late_append() {
    ...
    for _ in 0..10_001 {
        sink.append_run(run_id, LogLine::now(LogStream::Stdout, "repeat"))
            .await
            .unwrap();
    }
    ...
    let sink = InMemoryLogSink::with_log_dir(directory.clone());
    assert_eq!(
        sink.tail_run(run_id, 0, None, None).await.high_watermark,
        10_001
    );
...
async fn run_cursor_recovers_past_the_memory_window_before_applying_limit() {
    let directory =
        std::env::temp_dir().join(format!("my-supervisor-run-cursor-{}", uuid::Uuid::new_v4()));
    tokio::fs::create_dir_all(&directory).await.unwrap();
    let run_id = JobRunId::new();
    let sink = InMemoryLogSink::with_log_dir(directory.clone());
    ...
    for sequence in 1..=10_001 {
        sink.append_run(
            run_id,
            LogLine::now(LogStream::Stdout, format!("line-{sequence}")),
        )
        .await
        .unwrap();
    }
...
async fn legacy_collision_is_quarantined_and_singleton_history_migrates_once() {
    ...
    singleton
        .append("a/b", LogLine::now(LogStream::Stdout, "new"))
        .await
        .unwrap();
    drop(singleton);
