...
async fn deleting_run_history_commits_cleanup_queue_with_the_row_delete() {
    ...
    store
        .save_run(&JobRun {
            ...
            started_at: None,
            ended_at: Some(Utc::now() - Duration::days(2)),
            exit_code: Some(0),
    ...
    assert_eq!(
        store
            .prune_runs(&job.name, None, Some(Utc::now()))
            .await
            .unwrap(),
        vec![run_id]
    );
    ...
    assert_eq!(
        store.pending_run_log_cleanup(10).await.unwrap()[0].run_id,
        run_id
    );
    store
        .fail_run_log_cleanup(run_id, "permission denied")
        .await
        .unwrap();
    assert_eq!(
        store.pending_run_log_cleanup(10).await.unwrap()[0].attempts,
        1
    );
...
async fn run_filters_are_applied_before_the_limit() {
    ...
    for offset in 0..3 {
        ...
        store
            .save_run(&JobRun {
                ...
                job_id: job.id,
                triggered_by: TriggeredBy::Manual,
                scheduled_at: base + Duration::seconds(offset),
                started_at: None,
                ended_at: Some(base + Duration::seconds(offset)),
                exit_code: Some(0),
                state,
                occurrence: None,
                original_scheduled_at: None,
    ...
    let runs = store
        .list_runs_filtered(&job.name, Some(JobRunState::Failed), None, 2)
        .await
        .unwrap();
...
async fn fresh_schema_rejects_orphan_runs_and_stale_run_updates() {
    ...
    assert!(matches!(
        store.save_run(&orphan).await,
        Err(my_supervisor_core::ports::error::RepoError::Conflict(_))
    ));
    ...
    assert!(matches!(
        store.save_run(&persisted_run).await,
        Err(my_supervisor_core::ports::error::RepoError::Conflict(_))
    ));
}
...
async fn runtime_handle_cleanup_preserves_a_replacement_generation() {
    ...
    let old_handle = my_supervisor_core::domain::ChildHandle {
        ...
        pid: 101,
        pgid: Some(101),
        generation: Some("macos-libproc:10:1".into()),
        started_at: Utc::now(),
    };
    let new_handle = my_supervisor_core::domain::ChildHandle {
        ...
        pid: 102,
        pgid: Some(102),
        generation: Some("macos-libproc:10:2".into()),
        started_at: Utc::now(),
    };
    store
        .set_runtime_handle(&spec.name, Some(&old_handle))
        .await
        .unwrap();
    store
        .enqueue_runtime_handle_cleanup(&spec.name, &old_handle, "injected write failure")
        .await
        .unwrap();
    store
        .set_runtime_handle(&spec.name, Some(&new_handle))
        .await
        .unwrap();
    let cleanup = store
        .pending_runtime_handle_cleanup(1)
        .await
        .unwrap()
        .pop()
        .unwrap();
    assert!(!store
        ...
        .unwrap());
    assert_eq!(
        store.get_runtime_handle(&spec.name).await.unwrap(),
        Some(new_handle)
    );
}
