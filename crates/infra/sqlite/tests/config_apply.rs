...
use my_supervisor_core::domain::{
    ApplyMode, ConfigApplyJournal, ConfigApplyStage, ConfigDiff, ConfigSnapshot, ConfigTargetDirectStart,
    ChildHandle, DependencyFailurePolicy, DependencySignature, Job, JobDeletionJournal, JobDeletionStage, JobId, JobRun, JobRunId, JobRunState,
    JobTrigger, LogRetention, OverlapPolicy, ProcessSpec, TriggeredBy,
};
...
fn job(name: &str) -> Job {
    Job {
        id: JobId::new(), name: name.into(), command: "/bin/true".into(), args: Vec::new(),
        cwd: None, env: BTreeMap::new(), trigger: JobTrigger::Interval(std::time::Duration::from_secs(60)),
...
async fn config_snapshot_apply_and_journal_restore_are_atomic() {
    ...
    let previous = ConfigSnapshot { processes: vec![process("old")], jobs: vec![job("old-job")], running_direct_processes: vec![] };
    store.apply_config_snapshot(&previous).await.unwrap();
    let target = ConfigSnapshot { processes: vec![process("new")], jobs: vec![job("new-job")], running_direct_processes: vec![] };
    let apply_id = uuid::Uuid::new_v4();
    store.create_config_apply_journal(&ConfigApplyJournal {
        apply_id, previous: previous.clone(), target: target.clone(), diff: ConfigDiff::default(),
        stage: ConfigApplyStage::Prepared, compensation_error: None, target_direct_starts: Vec::new(),
    }).await.unwrap();
...
async fn config_target_start_intent_and_generation_survive_reopen() {
    let directory = std::env::temp_dir().join(format!("my-supervisor-config-target-start-{}", uuid::Uuid::new_v4()));
    ...
    store.create_config_apply_journal(&ConfigApplyJournal {
        apply_id,
        previous: ConfigSnapshot { processes: vec![target.clone()], jobs: Vec::new(), running_direct_processes: vec![target.name.clone()] },
        target: ConfigSnapshot { processes: vec![target.clone()], jobs: Vec::new(), running_direct_processes: vec![target.name.clone()] },
    ...
    store.record_config_target_direct_start(apply_id, &ConfigTargetDirectStart {
        name: target.name.clone(), spec: target.clone(), expected_generation: Some("macos-libproc:1:2".into()),
    }).await.unwrap();
    drop(store);

    let reopened = SqliteStore::connect(&database).await.unwrap();
    let journal = reopened.list_incomplete_config_applies().await.unwrap().pop().unwrap();
    assert_eq!(journal.target_direct_starts, vec![ConfigTargetDirectStart {
        name: target.name.clone(), spec: target, expected_generation: Some("macos-libproc:1:2".into()),
    }]);
    drop(reopened);
...
async fn dependency_signature_claim_creates_one_pending_run() {
    ...
    let signature = DependencySignature { run_ids: vec![JobRunId::new()] };
    let run = JobRun {
        run_id: JobRunId::new(), job_name: downstream.name.clone(), job_id: downstream.id,
        triggered_by: TriggeredBy::Manual, scheduled_at: Utc::now(), started_at: None, ended_at: None,
        exit_code: None, state: JobRunState::Pending,
    };
    assert!(store.claim_dependency_run(&downstream.name, &signature, &run).await.unwrap());
    assert!(!store.claim_dependency_run(&downstream.name, &signature, &run).await.unwrap());
    assert_eq!(store.list_runs(&downstream.name, 10).await.unwrap().len(), 1);
}

#[tokio::test]
async fn transient_cleanup_ticket_is_durable_and_stage_updates_are_idempotent() {
    let directory = std::env::temp_dir().join(format!("my-supervisor-terminal-outbox-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&directory).unwrap();
    let database = directory.join("state.db");
    let store = SqliteStore::connect(&database).await.unwrap();
    ...
    let run = JobRun {
        run_id: JobRunId::new(), job_name: parent.name.clone(), job_id: parent.id,
        triggered_by: TriggeredBy::Manual, scheduled_at: Utc::now(), started_at: Some(Utc::now()),
        ended_at: None, exit_code: None, state: JobRunState::Running,
    };
    store.save_run(&run).await.unwrap();
    let ticket = CleanupTicket {
        cleanup_id: uuid::Uuid::new_v4(), job_id: parent.id, job_name: parent.name.clone(), run_id: run.run_id,
        child: ChildHandle { process_id: uuid::Uuid::new_v4(), pid: 4242, pgid: Some(4242), generation: Some("test-generation".into()), started_at: Utc::now() },
        stage: TransientCleanupStage::JoinPumps, attempts: 0, last_error: Some("first failure".into()),
        intended_terminal_state: JobRunState::Cancelled,
        outcome: my_supervisor_core::ports::TransientOutcome { started_at: Utc::now(), ended_at: Utc::now(), exit_code: Some(23) },
    };
    ...
    store.update_transient_cleanup(&pending[0], TransientCleanupStage::PersistTerminal, None).await.unwrap();
    let pending = store.pending_transient_cleanup(10).await.unwrap();
    ...
    store.commit_transient_cleanup_terminal(&ticket, &terminal_run).await.unwrap();
    let persisted_run = store.get_run(&parent.name, &run.run_id).await.unwrap().unwrap();
    ...
    assert!(store.acknowledge_transient_terminal_event(event_id, ticket.cleanup_id).await.is_err());
    ...
    let reopened = SqliteStore::connect(&database).await.unwrap();
    let replay = reopened.pending_transient_terminal_events(10).await.unwrap();
    assert_eq!(replay.len(), 1);
    assert_eq!(replay[0].event_id, event_id);
    assert_eq!(replay[0].occurred_at, occurred_at);
    reopened.acknowledge_transient_terminal_event(event_id, ticket.cleanup_id).await.unwrap();
    assert!(reopened.pending_transient_cleanup(10).await.unwrap().is_empty());
    assert!(reopened.pending_transient_terminal_events(10).await.unwrap().is_empty());
    drop(reopened);
    std::fs::remove_dir_all(directory).unwrap();
}
...
async fn job_deletion_journal_survives_stage_updates_and_clears_after_completion() {
    ...
    let journal = JobDeletionJournal {
        deletion_id: uuid::Uuid::new_v4(), job: deleted_job.clone(),
        stage: JobDeletionStage::Prepared, run_ids: Vec::new(), last_error: None,
    };
    store.create_job_deletion_journal(&journal).await.unwrap();
    store.create_job_deletion_journal(&journal).await.unwrap();
    let persisted = store.get_job_deletion_journal(&deleted_job.name).await.unwrap().unwrap();
    assert_eq!(persisted.deletion_id, journal.deletion_id);
    let run_id = JobRunId::new();
    store.update_job_deletion_journal(
        journal.deletion_id, JobDeletionStage::RowsDeleted, Some(&[run_id]), Some("db retry"),
    ).await.unwrap();
    ...
    store.clear_job_deletion_journal(journal.deletion_id).await.unwrap();
...
async fn job_row_deletion_commits_rows_log_cleanup_and_journal_together_across_reopen() {
    let directory = std::env::temp_dir().join(format!("my-supervisor-deletion-commit-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&directory).unwrap();
    let database = directory.join("state.db");
    let store = SqliteStore::connect(&database).await.unwrap();
    ...
    assert!(store.commit_job_deletion_rows(journal.deletion_id, &deleting_job.name).await.is_err());
    assert!(store.get_job(&deleting_job.name).await.unwrap().is_some());
    assert_eq!(store.list_runs(&deleting_job.name, 10).await.unwrap().len(), run_ids.len());
    assert_eq!(
    ...
    let committed_run_ids = store.commit_job_deletion_rows(journal.deletion_id, &deleting_job.name).await.unwrap();
    assert_eq!(committed_run_ids.len(), run_ids.len());
    drop(store);

    let reopened = SqliteStore::connect(&database).await.unwrap();
    assert!(reopened.get_job(&deleting_job.name).await.unwrap().is_none());
    assert!(reopened.list_runs(&deleting_job.name, 10).await.unwrap().is_empty());
    let persisted = reopened.get_job_deletion_journal(&deleting_job.name).await.unwrap().unwrap();
    ...
    for cleanup in pending {
        reopened.complete_run_log_cleanup(cleanup.run_id).await.unwrap();
    }
    reopened.clear_job_deletion_journal(journal.deletion_id).await.unwrap();
    assert!(reopened.pending_run_log_cleanup(10).await.unwrap().is_empty());
    assert!(reopened.list_incomplete_job_deletions().await.unwrap().is_empty());
    drop(reopened);
    std::fs::remove_dir_all(directory).unwrap();
}
...
async fn queued_job_deletion_cancellation_is_atomic_with_its_journal_boundary() {
    let directory = std::env::temp_dir().join(format!("my-supervisor-delete-terminal-outbox-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&directory).unwrap();
    let database = directory.join("state.db");
    let store = SqliteStore::connect(&database).await.unwrap();
    ...
    assert!(store.cancel_queued_runs_for_job_deletion(
        journal.deletion_id,
        &deleting_job.name,
        &terminal_events,
    ).await.is_err());
    ...
    for run_id in queued_runs {
        assert_eq!(
            store.get_run(&deleting_job.name, &run_id).await.unwrap().unwrap().state,
            JobRunState::Pending,
        );
    }
    ...
    let reopened = SqliteStore::connect(&database).await.unwrap();
    reopened.cancel_queued_runs_for_job_deletion(
        journal.deletion_id,
        &deleting_job.name,
        &terminal_events,
    ).await.unwrap();
    assert_eq!(
        reopened.get_job_deletion_journal(&deleting_job.name).await.unwrap().unwrap().stage,
        JobDeletionStage::CancellationStarted,
    );
    for run_id in queued_runs {
        let run = reopened.get_run(&deleting_job.name, &run_id).await.unwrap().unwrap();
        assert_eq!(run.state, JobRunState::Cancelled);
        assert!(run.ended_at.is_some());
    }
    assert_eq!(reopened.pending_transient_terminal_events(10).await.unwrap(), terminal_events);
    drop(reopened);
    std::fs::remove_dir_all(directory).unwrap();
}
...
async fn migration_rebuilds_job_runs_when_job_id_exists_without_the_composite_fk() {
    let path = std::env::temp_dir().join(format!("my-supervisor-fk-migration-{}.sqlite", uuid::Uuid::new_v4()));
    let pool = SqlitePoolOptions::new()
        .max_connections(1)
        .connect_with(SqliteConnectOptions::new().filename(&path).create_if_missing(true).foreign_keys(true))
        .await
        .unwrap();
    sqlx::query("CREATE TABLE jobs (name TEXT PRIMARY KEY, id TEXT NOT NULL)").execute(&pool).await.unwrap();
    ...
    sqlx::query("INSERT INTO jobs(name, id) VALUES ('legacy', ?)").bind(job_id.0.to_string()).execute(&pool).await.unwrap();
    ...
    assert!(store.get_run("legacy", &valid_run_id).await.unwrap().is_some());
    assert!(store.get_run("legacy", &stale_run_id).await.unwrap().is_none());
    drop(store);

    let inspected = SqlitePoolOptions::new().max_connections(1)
        .connect_with(SqliteConnectOptions::new().filename(&path).foreign_keys(true)).await.unwrap();
    let foreign_keys = sqlx::query("PRAGMA foreign_key_list(job_runs)").fetch_all(&inspected).await.unwrap();
    ...
    assert!(sqlx::query("PRAGMA foreign_key_check").fetch_all(&inspected).await.unwrap().is_empty());
