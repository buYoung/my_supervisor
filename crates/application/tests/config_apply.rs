...
use my_supervisor_core::domain::{
    ...
    LifecycleMode, LoadedConfig, LogLine, LogRetention, OverlapPolicy, ProcessResourceUsage,
    ProcessSpec,
};
use my_supervisor_core::ports::error::{ConfigError, RegistrarError, SchedulerError};
use my_supervisor_core::ports::{
    Aliveness, CleanupTicket, ConfigSource, JobRepository, LifecycleController, ProbeError,
    ProcessServiceRegistrar, RealClock, ReapError, ScheduleEvent, ScheduledJob, Scheduler,
    SchedulerSnapshot, ShutdownSignaler, SignalError, SpawnError, StateRepository,
    TransientCompletion,
};
...
impl LifecycleController for NoopLifecycle {
    ...
    async fn probe_alive(&self, _handle: &ChildHandle) -> Result<Aliveness, ProbeError> {
        Ok(Aliveness::Dead)
    }
    ...
    async fn subscribe_detached_logs(
        ...
        Err(ProbeError::Failed("unexpected log subscribe".into()))
    }
    async fn resource_usage(
        &self,
        _handle: &ChildHandle,
    ) -> Result<ProcessResourceUsage, ProbeError> {
        Ok(ProcessResourceUsage::default())
    }
    async fn reap_on_shutdown(&self, _handles: &[ChildHandle]) -> Result<(), ReapError> {
        Ok(())
    }
    ...
    fn spawn(&self, spec: &ProcessSpec) -> Result<ChildHandle, SpawnError> {
            ...
            .is_ok()
        {
            return Err(SpawnError::Io {
                ...
                message: "injected target start failure".into(),
            });
        }
...
impl ShutdownSignaler for TrackingShutdown {
    async fn request_graceful(
        &self,
        target: &ChildHandle,
        _cfg: &my_supervisor_core::domain::ShutdownPolicy,
    ) -> Result<(), SignalError> {
...
impl LifecycleController for TrackingLifecycle {
    ...
    async fn probe_alive(&self, handle: &ChildHandle) -> Result<Aliveness, ProbeError> {
        Ok(
            if self.alive.lock().unwrap().contains_key(&handle.process_id) {
                Aliveness::Alive
            } else {
                Aliveness::Dead
            },
...
impl ShutdownSignaler for NoopShutdown {
    ...
    async fn force_kill(&self, _target: &ChildHandle) -> Result<(), SignalError> {
        Ok(())
    }
}
...
impl PreparedRegistrar {
    ...
    fn was_unregistered(&self, unit_name: &str) -> bool {
        self.unregistered_units
            .lock()
            .unwrap()
            .iter()
            .any(|candidate| candidate == unit_name)
    }
}
...
impl ProcessServiceRegistrar for PreparedRegistrar {
    ...
    async fn query_status(
        &self,
        _unit_name: &str,
    ) -> Result<my_supervisor_core::domain::ProcessState, RegistrarError> {
...
impl Scheduler for ControlledScheduler {
    async fn register(
        &self,
        name: &str,
        trigger: &my_supervisor_core::domain::JobTrigger,
    ) -> Result<(), SchedulerError> {
        if self.fail_next_register.swap(false, Ordering::SeqCst) {
            return Err(SchedulerError::Backend("injected scheduler failure".into()));
        }
        self.entries
            .lock()
            .unwrap()
            .insert(name.into(), trigger.clone());
        Ok(())
    }
    async fn unregister(&self, name: &str) -> Result<(), SchedulerError> {
    ...
    async fn restore(&self, snapshot: &SchedulerSnapshot) -> Result<(), SchedulerError> {
        *self.entries.lock().unwrap() = snapshot
            .entries
            .iter()
            .map(|entry| (entry.name.clone(), entry.trigger.clone()))
            .collect();
        Ok(())
    }
    fn next_run(
        &self,
        _trigger: &my_supervisor_core::domain::JobTrigger,
...
fn config(name: &str) -> LoadedConfig {
    LoadedConfig {
        processes: vec![ProcessSpec::new(name, "/bin/true")],
        jobs: Vec::new(),
    }
}
...
async fn dry_run_is_side_effect_free_and_replace_leaves_no_journal() {
    ...
    facade
        .apply_config(config("old"), ApplyMode::Merge, false)
        .await
        .unwrap();
    let dry_run = facade
        .apply_config(config("new"), ApplyMode::Replace, true)
        .await
        .unwrap();
    ...
    facade
        .apply_config(config("new"), ApplyMode::Replace, false)
        .await
        .unwrap();
...
async fn prepare_failure_rolls_back_but_post_commit_start_failure_preserves_forward_recovery() {
    ...
    assert!(facade
        .apply_config(scheduler_failure, ApplyMode::Replace, false)
        .await
        .is_err());
    assert!(facade.get_process("old").await.is_ok());
    assert!(facade.get_process("new").await.is_err());
    ...
    let error = facade
        .apply_config(start_failure, ApplyMode::Replace, false)
        .await
        .unwrap_err();
...
async fn same_label_prepare_failure_preserves_old_unit_and_start_failure_is_forward_recovery() {
    ...
    old.autostart = true;
    old.management_mode = my_supervisor_core::domain::ManagementMode::SystemRegistered {
        unit_name: label.into(),
    };
    facade
        .apply_config(
            LoadedConfig {
                processes: vec![old.clone()],
                jobs: Vec::new(),
            },
            ApplyMode::Replace,
            false,
    ...
    failing_prepare.management_mode =
        my_supervisor_core::domain::ManagementMode::SystemRegistered {
            unit_name: "com.example.prepare-failure".into(),
        };
    registrar.fail_registration_for("com.example.prepare-failure");
    assert!(facade
        .apply_config(
            LoadedConfig {
                processes: vec![changed.clone(), failing_prepare],
                jobs: Vec::new()
            },
            ApplyMode::Replace,
            false
        ...
        .is_err());
    assert!(
        registrar.is_running(label),
        "same-label prepare failure must not stop the old unit"
    );
    ...
    *registrar.failing_registration.lock().unwrap() = None;
    ...
    assert!(
        registrar.is_running(label),
        "same-label live unit remains until a successful replacement start"
    );
    assert!(!registrar.was_unregistered(label));
    assert_eq!(
        store.list_incomplete_config_applies().await.unwrap()[0].stage,
...
async fn forward_recovery_restarts_previously_running_autostart_false_target_after_start_failure() {
    ...
    assert!(
        !first_lifecycle.is_pid_alive(old_pid),
        "the old target PID must be reaped before forward recovery"
    );
    let journal = store
        .list_incomplete_config_applies()
        .await
    ...
    let recovered_facade = OperationsFacade::new(config_dependencies(
        store.clone(),
        recovered_lifecycle.clone(),
    ));
    recovered_facade
        .recover_incomplete_config_apply()
        .await
        .unwrap();

    assert_eq!(
        recovered_lifecycle.spawn_count(),
        1,
        "the target must start once even with autostart=false"
    );
    assert_eq!(
        recovered_facade.get_process("worker").await.unwrap().state,
        my_supervisor_core::domain::ProcessState::Running
    );
    assert!(store
        .list_incomplete_config_applies()
        .await
...
async fn forward_recovery_recognizes_generation_recorded_before_crash_without_duplicate_spawn() {
    ...
    target.autostart = false;
    store
        .apply_config_snapshot(&ConfigSnapshot {
            processes: vec![target.clone()],
            jobs: Vec::new(),
            running_direct_processes: Vec::new(),
        })
        .await
        .unwrap();
    let handle = lifecycle.spawn_detached(&target).await.unwrap();
    store
        .set_runtime_handle(&target.name, Some(&handle))
        .await
        .unwrap();
    ...
    store
        .create_config_apply_journal(&ConfigApplyJournal {
            ...
            target_direct_starts: vec![ConfigTargetDirectStart {
                ...
                spec: target.clone(),
                expected_generation: handle.generation.clone(),
            }],
        })
        .await
        .unwrap();
    ...
    let recovered_facade =
        OperationsFacade::new(config_dependencies(store.clone(), lifecycle.clone()));
    recovered_facade
        .recover_incomplete_config_apply()
        .await
        .unwrap();
    ...
    assert_eq!(
        lifecycle.spawn_count(),
        1,
        "recovery must restore the verified target instead of spawning another"
    );
    assert_eq!(
        recovered_facade.get_process("worker").await.unwrap().pid,
        Some(handle.pid)
    );
    assert!(store
        .list_incomplete_config_applies()
...
async fn replace_removes_a_previously_running_direct_process_from_the_runtime_target() {
    ...
    assert!(
        !lifecycle.is_pid_alive(old_pid),
        "a removed target must not retain its old PID"
    );
