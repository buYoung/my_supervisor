...
**결정:** `crates/desktop/ui/src/` 를 `features/*` + `components/ui` + `services/` + `shared/` 로 구성한다. 각 feature (processes, logs, daemon, settings) 는 자기 UI · 자기 service 호출 · 자기 훅을 자체 디렉터리에 둔다.
...
**운영 규약:**
- 토큰 단일 출처: `crates/desktop/ui/src/shared/theme.css` 의 `:root` / `[data-theme="dark"]`
