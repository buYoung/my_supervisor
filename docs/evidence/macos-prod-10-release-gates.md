...
- verifier와 독립된 read-only comparison도 source/app/mounted-DMG의 같은 다섯 binary byte equality, exact `arm64`, 위 SHA-256을 재확인했다. normal/negative verifier와 independent comparison은 생성한 exact device만 detach하고 mount directory removal을 관찰했다. post-negative `hdiutil info`에는 `my-supervisor`, `package-security`, 독립 mount 항목이 없었다. 이 local unsigned verifier는 consistency, tamper, path, cleanup만 판정하며 ignored build artifact가 source-derived임을 암호학적으로 보장하지 않는다.
...
- same-path target swap은 사용자 build state와 비정상 종료 복구 위험 때문에 채택하지 않았다. `e9702b0` isolated rebuild 시도와 revert는 역사적 engineering note일 뿐 release gate가 아니다.
...
- `C01`: 저장소 CI 소유. clean hosted CI가 immutable revision에서 build하고 source revision·artifact digest를 signed attestation으로 결속한 성공 run은 관찰되지 않았다.
- `E01`: Apple release 소유. Developer ID signing/notarization/stapling은 credential, network와 별도 승인이 필요하다.
...
core process engine과 대표 CLI/GUI 운영 계약의 역사적 로컬 source gate 및 현재 unsigned arm64 후보의 P01–P03 local consistency/tamper/path/cleanup gate는 통과했다. source derivation은 unsigned local verifier가 아니라 C01 clean hosted CI의 immutable revision+artifact attestation 및 E01 Developer ID signing/notarization/stapling에서 완료되어야 한다. 따라서 production deployment는 준비되지 않았다. publish, upload, signing, notarization, stapling 또는 deployment를 수행했다는 주장은 하지 않는다.
