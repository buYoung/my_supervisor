...
## Current State (As-Is)
- Every view imports static mock arrays directly: `crates/desktop/ui/src/features/processes/ProcessesView.tsx`, `crates/desktop/ui/src/features/jobs/JobsView.tsx`, and `crates/desktop/ui/src/features/logs/LogsView.tsx` all read from `crates/desktop/ui/src/shared/mock-data.ts`. There is no `services/` layer, transport client, or state library.
- `crates/desktop/ui/src/App.tsx` hardcodes the daemon-connected indicator (`127.0.0.1:9876`, "데몬 연결됨") and renders 5 tabs (`processes/jobs/logs/daemon/settings`) from `NavigationKey`.
- `crates/desktop/ui/src/shared/types.ts` defines camelCase shapes (`ProcessStatus`, `JobStatus`, `JobRun`, `LogLine`, `DaemonStatus`), but the backend wire (`docs/API.md` §4) is snake_case (`restart_count`, `started_at`, `triggered_by`, …) and `types.ts` carries derived/divergent fields (`ProcessStatus.uptime`; `JobRun.triggeredBy` flat vs the API's tagged `triggered_by`; the wire log line is `{timestamp, stream, line}` only, but `LogLine` adds `id`/`source`) — so the client must map between them.
...
- A `services/` layer under `crates/desktop/ui/src` exposes a **transport-agnostic** operations client with two adapters behind one interface: `invoke` (used when running inside Tauri — the production path) and HTTP + WS (used standalone against the daemon). The three views consume the interface, not a specific transport.
...
### Out of Scope
- [hard] Do not change the visual design system or the `crates/desktop/ui/src/components/ui/primitives.tsx` API — wire data through existing components.
...
- Keep `crates/desktop/ui/src/shared/types.ts` as the single source of FE types; both adapters map the snake_case wire onto these camelCase shapes — do not fork shapes or push camelCase onto the wire.

## Related Files / Entry Points
- `crates/desktop/ui/src/shared/mock-data.ts` — the current data source to replace with the live client.
- `crates/desktop/ui/src/shared/types.ts` — wire types to keep aligned with the backend `shared` crate.
- `crates/desktop/ui/src/features/processes/ProcessesView.tsx` — Processes view to wire.
- `crates/desktop/ui/src/features/jobs/JobsView.tsx` — Jobs view to wire.
- `crates/desktop/ui/src/features/logs/LogsView.tsx` — Logs view to wire to per-process log-follow.
- `crates/desktop/ui/src/App.tsx` — header indicator to wire; nav stays at 5 tabs this slice.
...
- [ ] Any `crates/desktop/ui/src/shared/types.ts` field change stays consistent with the snake_case wire of the Rust `shared` crate.
