...
## Current State (As-Is)
- No Tauri scaffolding exists: a repo scan finds no `src-tauri`, no `tauri.conf.json`, and no `.rs` files. The `apps/my_supervisor/desktop/src-tauri` host from `docs/ARCHITECTURE.md` §4.1.3 is unimplemented.
- `apps/my_supervisor/desktop` is a Vite + React 19 app currently runnable standalone (`pnpm -C apps/my_supervisor/desktop dev`) on mock data; it is not yet loaded by any native shell.
...
- `apps/my_supervisor/desktop/src-tauri` is a Tauri v2 binary that calls child 01's composition function in-process and hosts a WebView loading `apps/my_supervisor/desktop`.
...
## Scope
### In Scope
- `apps/my_supervisor/desktop/src-tauri` Tauri v2 crate plus `tauri.conf.json`, with `frontendDist` / dev URL pointed at `apps/my_supervisor/desktop`.
...
- `apps/my_supervisor/desktop/src-tauri/` (proposed) — new Tauri host crate location.
- `apps/my_supervisor/desktop/package.json` — frontend scripts and the build the WebView loads.
- `apps/my_supervisor/desktop/vite.config.ts` — dev server URL and build output dir the Tauri config must point at.
...
## Acceptance Criteria
- [ ] `cargo tauri dev` launches the desktop app loading `apps/my_supervisor/desktop` in a WebView, with the UI driving operations via `invoke`.
...
- Exact crate/dir layout for the Tauri host — follow the docs' `apps/my_supervisor/desktop/src-tauri` placement vs Tauri's conventional `src-tauri`? (recommend follow the docs and adapt the Tauri config)
