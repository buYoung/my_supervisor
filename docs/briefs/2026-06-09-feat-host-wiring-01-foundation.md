...
## Current State (As-Is)
- At the time this brief was first drafted, `crates/` held only `crates/.gitkeep` and no Rust workspace existed. The current repository now has a root `Cargo.toml` with workspace members under `crates/`.
...
## Scope
### In Scope
- `Cargo.toml` workspace with member globs per `docs/DEVELOPMENT.md` §3 (`crates/core`, `crates/application`, `crates/shared`, `crates/config`, `crates/infra/*`, `crates/platform/*`, `crates/daemon`, `crates/cli`, `crates/desktop`).
...
- `shared`: wire DTOs matching `crates/desktop/ui/src/shared/types.ts` and `docs/API.md` §4, plus the error envelope from `docs/API.md` §5.
...
- `crates/desktop/ui/src/shared/types.ts` — the TS wire shapes the Rust `shared` crate must mirror.
