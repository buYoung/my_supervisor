...
## Scope
### In Scope
- `apps/my_supervisor/Cargo.toml` workspace with member globs per `docs/DEVELOPMENT.md` §3 (`crates/core`, `crates/application`, `crates/shared`, `crates/config`, `crates/infra/*`, `crates/platform/*`, `crates/daemon`, `crates/cli`, `crates/desktop`).
...
- `shared`: wire DTOs matching `apps/my_supervisor/crates/desktop/ui/src/shared/types.ts` and `docs/API.md` §4, plus the error envelope from `docs/API.md` §5.
...
- `apps/my_supervisor/crates/desktop/ui/src/shared/types.ts` — the TS wire shapes the Rust `shared` crate must mirror.
