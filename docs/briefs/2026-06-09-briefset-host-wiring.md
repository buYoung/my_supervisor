...
- `packages/ui/src/shared/types.ts` ↔ `crates/shared` wire DTOs — the parity surface (children 01, 04, 06).
- `packages/ui/src/features/processes/ProcessesView.tsx` — operations-data wiring (child 04) and the SystemRegistered convert flow (child 06) both edit it. Serialize.
- `packages/ui/dist` build output — produced by child 04, embedded by child 03.
