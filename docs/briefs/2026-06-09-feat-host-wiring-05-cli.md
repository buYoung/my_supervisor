...
## Current State (As-Is)
- `crates/app/cli` does not exist; the `msv` binary described in `docs/ARCHITECTURE.md` §4.1.2 and `docs/DEVELOPMENT.md` §3 is unimplemented.
...
- `crates/app/cli` is a binary (`msv`) that drives the operations HTTP API over a configurable base URL: process and job listing/lifecycle, and log tailing, from the terminal.
...
## Scope
### In Scope
- `crates/app/cli` binary crate + workspace member entry; CLI argument parsing.
...
- `crates/app/cli/` (proposed) — new CLI binary crate location.
