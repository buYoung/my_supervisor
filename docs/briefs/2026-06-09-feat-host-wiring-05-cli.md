...
## Current State (As-Is)
- `apps/my_supervisor/crates/cli` does not exist; the `msv` binary described in `docs/ARCHITECTURE.md` §4.1.2 and `docs/DEVELOPMENT.md` §3 is unimplemented.
...
- `apps/my_supervisor/crates/cli` is a binary (`msv`) that drives the operations HTTP API over a configurable base URL: process and job listing/lifecycle, and log tailing, from the terminal.
...
## Scope
### In Scope
- `apps/my_supervisor/crates/cli` binary crate + workspace member entry; CLI argument parsing.
...
- An HTTP/WS client reusing `apps/my_supervisor/crates/shared` DTOs; a `--url` base-URL override; the `-o json` output mode (per §4.1.2).
...
## Constraints
- Reuse `apps/my_supervisor/crates/shared` DTOs; do not fork wire types or hand-roll JSON shapes.
...
- `apps/my_supervisor/crates/cli/` (proposed) — new CLI binary crate location.
...
- [ ] The CLI imports `apps/my_supervisor/crates/shared` DTOs directly — no duplicated type definitions.
